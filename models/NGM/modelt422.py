...
class Net(CNN):
    def __init__(self, double=True):
        ...
        if cfg.NGM.EDGE_FEATURE == 'cat':
            if double:
                self.affinity_layer = InnerpAffinity(cfg.NGM.FEATURE_CHANNEL * 2)
            else:
...
class IANet(CNN):
    def __init__(self):
        ...
        self.t1 = Net(double=False)
        self.t2 = Net()
        self.t3 = Net()
        self.t4 = Net()

    def forward(self, data_dict, alpha=1, **kwargs):
        data_dict = self.t1(data_dict)
        s1 = data_dict['ds_mat']
        U_src0, U_tgt0, F_src0, F_tgt0 = data_dict['UFs']
        ...
        U_srcC1 = torch.cat((U_src1, U_srcT1), 1)
        U_tgtC1 = torch.cat((U_tgt1, U_tgtT1), 1)
        F_srcC1 = torch.cat((F_src1, F_srcT1), 1)
        F_tgtC1 = torch.cat((F_tgt1, F_tgtT1), 1)
        data_dict['UFs'] = (U_srcC1, U_tgtC1, F_srcC1, F_tgtC1)

        data_dict = self.t2(data_dict)
        s2 = data_dict['ds_mat']
        # U_src0, U_tgt0, F_src0, F_tgt0 = data_dict['UFs']
        U_src1 = torch.bmm(s2, U_tgt0.transpose(2, 1)).transpose(2, 1)
        U_tgt1 = torch.bmm(s2.transpose(2, 1), U_src0.transpose(2, 1)).transpose(2, 1)
        F_src1 = torch.bmm(s2, F_tgt0.transpose(2, 1)).transpose(2, 1)
        F_tgt1 = torch.bmm(s2.transpose(2, 1), F_src0.transpose(2, 1)).transpose(2, 1)
        U_srcT1 = alpha * U_src0 + (1 - alpha) * U_src1
        U_tgtT1 = alpha * U_tgt0 + (1 - alpha) * U_tgt1
        F_srcT1 = alpha * F_src0 + (1 - alpha) * F_src1
        F_tgtT1 = alpha * F_tgt0 + (1 - alpha) * F_tgt1
        ...
        s3 = data_dict['ds_mat']
        # U_src0, U_tgt0, F_src0, F_tgt0 = data_dict['UFs']
        U_src1 = torch.bmm(s3, U_tgt0.transpose(2, 1)).transpose(2, 1)
        U_tgt1 = torch.bmm(s3.transpose(2, 1), U_src0.transpose(2, 1)).transpose(2, 1)
        F_src1 = torch.bmm(s3, F_tgt0.transpose(2, 1)).transpose(2, 1)
        F_tgt1 = torch.bmm(s3.transpose(2, 1), F_src0.transpose(2, 1)).transpose(2, 1)
        U_srcT1 = alpha * U_src0 + (1 - alpha) * U_src1
        U_tgtT1 = alpha * U_tgt0 + (1 - alpha) * U_tgt1
        F_srcT1 = alpha * F_src0 + (1 - alpha) * F_src1
        F_tgtT1 = alpha * F_tgt0 + (1 - alpha) * F_tgt1
        ...
        s4 = data_dict['ds_mat']
        # U_src0, U_tgt0, F_src0, F_tgt0 = data_dict['UFs']
        U_src1 = torch.bmm(s4, U_tgt0.transpose(2, 1)).transpose(2, 1)
        U_tgt1 = torch.bmm(s4.transpose(2, 1), U_src0.transpose(2, 1)).transpose(2, 1)
        F_src1 = torch.bmm(s4, F_tgt0.transpose(2, 1)).transpose(2, 1)
        F_tgt1 = torch.bmm(s4.transpose(2, 1), F_src0.transpose(2, 1)).transpose(2, 1)
        U_srcT1 = alpha * U_src0 + (1 - alpha) * U_src1
        U_tgtT1 = alpha * U_tgt0 + (1 - alpha) * U_tgt1
        F_srcT1 = alpha * F_src0 + (1 - alpha) * F_src1
        F_tgtT1 = alpha * F_tgt0 + (1 - alpha) * F_tgt1
