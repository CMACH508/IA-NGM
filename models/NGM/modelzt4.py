...
class Net(CNN):
    ...
    def forward(self, data_dict, **kwargs):
        ...
        if 'images' in data_dict:
            ...
            # src_glob = self.final_layers(src_edge)
            # tgt_glob = self.final_layers(tgt_edge)
            ...
            # src_glob = self.l2norm(src_glob)
            # tgt_glob = self.l2norm(tgt_glob)
        ...
        if 'images' in data_dict or 'IA' in data_dict or 'features' in data_dict:
            ...
            if 'IA' in data_dict:
                K0 = data_dict['aff_mat']
                alpha = data_dict['alpha']
                K = (alpha * K0 + (2 - alpha) * K)/2
        ...
        if 'IA' not in data_dict:
            ...
            data_dict['IA'] = True
            # data_dict['global'] = torch.cat([src_glob, tgt_glob], axis=1)[:, :, 0, 0]
...
class IANet(CNN):
    ...
    def forward(self, data_dict, alpha=1.5, **kwargs):
        ...
        for i in range(self.k - 1):
            ...
            data_dict1['UFs'] = (U_src1, U_tgt1, F_src1, F_tgt1)
            #data_dict1['aff_mat'] = K0
            # data_dict1['aff_mat'] = (alpha * K0 + (2 - alpha) * data_dict1['aff_mat']) / 2
