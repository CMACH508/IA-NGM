...
class IANet(CNN):
    ...
    def forward(self, data_dict, alpha=0.5, **kwargs):
        ...
        if 'Fi' in data_dict1:
            ...
            for i in range(3):
                ...
                Fj1 = torch.bmm(s.transpose(1, 2), Fi)
                Fi1 = torch.bmm(s, Fj)
                # Fi2 = alpha * Fi0 + (1 - alpha) * Fj1
                # Fj2 = alpha * Fj0 + (1 - alpha) * Fi1
                aff_mat = kronecker_torch(Fj1, Fi1)
