...
class Net(CNN):
    ...
    def forward(
        ...
        if 'IA' in data_dict:
            ...
            alpha1 = data_dict['alpha1']
            alpha2 = data_dict['alpha2']
            s = data_dict['ds_mat']
            ...
            for b in range(batch_size):
                node_srcT1[b] = alpha1[b] * node_src[b] + alpha2[b] * node_src1[b]
                node_tgtT1[b] = alpha1[b] * node_tgt[b] + alpha2[b] * node_tgt1[b]
...
class IANet(CNN):
    def __init__(self):
        ...
        self.aw1 = torch.nn.Sequential(torch.nn.Linear(512, 32), torch.nn.Tanh(), torch.nn.Linear(32, 1),
                                       torch.nn.Tanh())
        self.aw2 = torch.nn.Sequential(torch.nn.Linear(512, 32), torch.nn.Tanh(), torch.nn.Linear(32, 1),
                                       torch.nn.Tanh())
    ...
    def forward(self, data_dict, alpha=0.75):
        ...
        data_dict['alpha1'] = self.aw1(global_info[0][:, :512])
        data_dict['alpha2'] = self.aw2(global_info[0][:, 512:])
