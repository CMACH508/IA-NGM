...
class GNNLayer(nn.Module):
    ...
    def forward(self, A, W, x, n1=None, n2=None, norm=True):
        ...
        if self.classifier is not None:
            ...
            x4 = x3.permute(0, 2, 1).reshape(x.shape[0] * self.sk_channel, int(n2.max()), int(n1.max())).transpose(1, 2)
...
class HyperConvLayer(nn.Module):
    ...
    def forward(self, H, E, x, n1=None, n2=None, norm=None):
        ...
        if self.classifier is not None:
            ...
            x5 = self.sk(x3.view(x.shape[0], n2.max(), n1.max()).transpose(1, 2), n1, n2, dummy_row=True).transpose(2,
                                                                                                                    1).contiguous()
