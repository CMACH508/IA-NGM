...
from src.build_graphs import reshape_edge_feature, construct_edge_feature
...
class IANet(CNN):
    ...
    def forward(self, fea_src, fea_tgt, G_src, G_tgt, H_src, H_tgt, ns_src, ns_tgt, K_G, K_H, inp_type='feat'):

        X = construct_edge_feature(fea_src, G_src, H_src)  # Xc = [Fi|Fj] batch_size * 2 feat_dim * edge_num
        Y = construct_edge_feature(fea_tgt, G_tgt, H_tgt)

        U_src = fea_src.transpose(1, 2)
        U_tgt = fea_tgt.transpose(1, 2)
        ...
        v = v.view(v.shape[0], fea_tgt.shape[1], -1).transpose(1, 2)
        s = self.sinkhorn(v, ns_src, ns_tgt)
        Bv = self.binary(s, ns_src, ns_tgt)
        return v, s, Bv
