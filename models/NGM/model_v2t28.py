...
class Net(CNN):
    ...
    def forward(
        ...
        if 'IA' in data_dict:
            global_weights_list0 = data_dict['global_weights_list']
            alpha = data_dict['alpha']
            s = data_dict['ds_mat']
            g_src, g_tgt = data_dict['orig_graph_list']
            node_src = torch.stack(pad_tensor([g.x for g in g_src]), dim=0)
            node_tgt = torch.stack(pad_tensor([g.x for g in g_tgt]), dim=0)
            node_src1 = torch.bmm(node_tgt.transpose(2, 1), s).transpose(2, 1)
            node_tgt1 = torch.bmm(node_src.transpose(2, 1), s.transpose(2, 1)).transpose(2, 1)

            node_srcT1 = torch.zeros_like(node_src)
            node_tgtT1 = torch.zeros_like(node_tgt)
            for b in range(batch_size):
                node_srcT1[b] = alpha[b] * node_src[b] + (1 - alpha[b]) * node_src1[b]
                node_tgtT1[b] = alpha[b] * node_tgt[b] + (1 - alpha[b]) * node_tgt1[b]
                #node_srcT1[b] = alpha * node_src[b] + (1 - alpha) * node_src1[b]
                #node_tgtT1[b] = alpha * node_tgt[b] + (1 - alpha) * node_tgt1[b]
                g_src[b].x = node_srcT1[b][:n_points[0][b], ]
                g_tgt[b].x = node_tgtT1[b][:n_points[1][b], ]
            g_srcx = torch.cat([x.x for x in g_src], dim=0)
            g_tgtx = torch.cat([x.x for x in g_tgt], dim=0)

            graphs[0].x = g_srcx
            graphs[0] = self.message_pass_node_features(graphs[0])
            orig_graph0 = self.build_edge_features_from_node_features(graphs[0])
            graphs[1].x = g_tgtx
            graphs[1] = self.message_pass_node_features(graphs[1])
            orig_graph1 = self.build_edge_features_from_node_features(graphs[1])
            for b in range(batch_size):
                #orig_graph0[b].edge_attr = g_src[b].edge_attr * alpha + (1 - alpha) * orig_graph0[b].edge_attr
                #orig_graph1[b].edge_attr = g_tgt[b].edge_attr * alpha + (1 - alpha) * orig_graph1[b].edge_attr
                orig_graph0[b].edge_attr = g_src[b].edge_attr * alpha[b] + (1 - alpha[b]) * orig_graph0[b].edge_attr
                orig_graph1[b].edge_attr = g_tgt[b].edge_attr * alpha[b] + (1 - alpha[b]) * orig_graph1[b].edge_attr

            orig_graph_list = [orig_graph0, orig_graph1]
            ...
            global_weights_list1 = [
                torch.cat([global_src, global_tgt], axis=-1) for global_src, global_tgt in lexico_iter(global_list)
            ]
            global_weights_list1 = [normalize_over_channels(g) for g in global_weights_list1]
            global_weights_list = [torch.zeros_like(global_weights_list1[0])]
            for b in range(batch_size):
                global_weights_list[0][b] = alpha[b] * global_weights_list0[0][b] + \
                                            (1 - alpha[b]) * global_weights_list1[0][b]
                #global_weights_list[0][b] = alpha * global_weights_list0[0][b] + \
                #                            (1 - alpha) * global_weights_list1[0][b]
        ...
        if cfg.PROBLEM.TYPE == '2GM':
            ...
            if 'IA' not in data_dict:
                data_dict['IA'] = True
                data_dict['global_weights_list'] = global_weights_list
                data_dict['orig_graph_list'] = orig_graph_list
...
class IANet(CNN):
    ...
    def forward(self, data_dict, alpha=0.75):
        data_dict['alpha'] = alpha
        data_dict = self.t1(data_dict)
        global_info = data_dict['global_weights_list']
        data_dict['alpha'] = self.aw(global_info[0])
