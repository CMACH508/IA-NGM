...
class Net(CNN):
    def __init__(self):
        ...
        self.aw = torch.nn.Sequential(torch.nn.Linear(2048, 32), torch.nn.Tanh(), torch.nn.Linear(32, 1),
                                      torch.nn.Tanh())
    ...
    def forward(
        ...
        if 'IA' in data_dict:
            ...
            alpha = self.aw(torch.cat((global_weights_list0[0], global_weights_list1[0]), dim=1))
...
class IANet(CNN):
    ...
    def forward(self, data_dict, alpha=0.75):
        ...
        # data_dict['alpha'] = self.t1.aw(global_info[0])
