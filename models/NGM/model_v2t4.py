...
class Net(CNN):
    ...
    def forward(
        ...
        if 'IA' in data_dict:
            global_weights_list = data_dict['global_weights_list']
            orig_graph_list = data_dict['orig_graph_list']
            Ns = data_dict['Ns']
            unary_affs_list = [
                self.vertex_affinity([item for item in g_1], [item for item in g_2], global_weights)
                for g_1, g_2, global_weights in zip([Ns[0]], [Ns[1]], global_weights_list)
            ]
        else:
            global_list = []
            orig_graph_list = []
            ...
            Ns = [([item.x for item in g_1], [item.x for item in g_2]) for (g_1, g_2) in lexico_iter(orig_graph_list)]
...
class IANet(CNN):
    ...
    def forward(self, data_dict, alpha=0.75):
        ...
        s1 = data_dict['ds_mat']
        Ns = data_dict['Ns']
        node_src, node_tgt = [pad_tensor(N) for N in Ns[0]]  # list8(batch_size)*tensor(num_node*1024(dim))
        node_src = torch.stack(node_src, dim=0)
        node_tgt = torch.stack(node_tgt, dim=0)
        node_src1 = torch.bmm(node_tgt.transpose(2, 1), s1).transpose(2, 1)
        node_tgt1 = torch.bmm(node_src.transpose(2, 1), s1.transpose(2, 1)).transpose(2, 1)
        node_srcT1 = alpha * node_src + (1 - alpha) * node_src1
        node_tgtT1 = alpha * node_tgt + (1 - alpha) * node_tgt1
        data_dict['Ns'] = [node_srcT1, node_tgtT1]

        data_dict = self.t2(data_dict)
        s2 = data_dict['ds_mat']
        node_src2 = torch.bmm(node_tgt.transpose(2, 1), s2).transpose(2, 1)
        node_tgt2 = torch.bmm(node_src.transpose(2, 1), s2.transpose(2, 1)).transpose(2, 1)
        node_srcT2 = alpha * node_src + (1 - alpha) * node_src2
        node_tgtT2 = alpha * node_tgt + (1 - alpha) * node_tgt2
        data_dict['Ns'] = [node_srcT2, node_tgtT2]

        data_dict = self.t3(data_dict)
        s3 = data_dict['ds_mat']
        node_src3 = torch.bmm(node_tgt.transpose(2, 1), s3).transpose(2, 1)
        node_tgt3 = torch.bmm(node_src.transpose(2, 1), s3.transpose(2, 1)).transpose(2, 1)
        node_srcT3 = alpha * node_src + (1 - alpha) * node_src3
        node_tgtT3 = alpha * node_tgt + (1 - alpha) * node_tgt3
        data_dict['Ns'] = [node_srcT3, node_tgtT3]

        data_dict = self.t4(data_dict)
        s4 = data_dict['ds_mat']
        node_src4 = torch.bmm(node_tgt.transpose(2, 1), s4).transpose(2, 1)
        node_tgt4 = torch.bmm(node_src.transpose(2, 1), s4.transpose(2, 1)).transpose(2, 1)
        node_srcT4 = alpha * node_src + (1 - alpha) * node_src4
        node_tgtT4 = alpha * node_tgt + (1 - alpha) * node_tgt4
        data_dict['Ns'] = [node_srcT4, node_tgtT4]
        return data_dict
