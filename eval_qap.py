...
def eval_model(model, alphas, dataloader, eval_epoch=None, verbose=False):
    ...
    for i, cls in enumerate(classes):
        ...
        for inputs in dataloader:
            ...
            ori_affmtx = inputs['aff_mat'].to(device)
            solution = inputs['solution'].to(device)
            name = inputs['name']
            ...
            for n, x, y, z, p, m in zip(name, obj_score, opt_obj_score, ori_obj_score, x_pred, affmtx):
                # if m == 'kra32':
                #     f = open('AFF1.txt', 'w')
                #     for i in range(m.shape[0]):
                #         f.write(' '.join([str(j) for j in m[i].cpu().numpy().tolist()]) + '\n')
                #     f.close()
